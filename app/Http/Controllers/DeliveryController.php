...
class DeliveryController extends Controller
{
    public function showDelivery(Request $request)
    {
        ...
        if ($curriculum->alway_delivery_flg == true) {
            $canView = true;
        } else {
            $canView = false;
            $modelDeliveryTime = new DeliveryTime();
            $deliveryTime = $modelDeliveryTime->getDeliveryTime($request->curriculum_id);
            foreach ($deliveryTime as $time) {
                if (now() >= $time->delivery_from && now() <= $time->delivery_to) {
                    $canView = true;
                }
            }
        }
